...
object CRDTActorMap {
  ...
  case class DeltaMsg(from: ActorRef[Command], delta: scala.collection.mutable.Map[String, Int]) extends Command
  ...
  case class LockAcquired(sender: ActorRef[Command]) extends CRDTActorMap.Command //Receive a lock from another actor

  case object LockReleased extends Command

  case class responseMsg(msg: scala.collection.mutable.Map[String, Int]) extends Command
  ...
  case class TerminateActor() extends Command
...
class CRDTActorMap(
  ...
  private var crdtstate = scala.collection.mutable.Map.empty[String, Int]
  private var tempCrdtState = scala.collection.mutable.Map.empty[String, Int]
  ...
  private def broadcastAndResetDeltas(): Unit =
    ...
    deltaOption match
      ...
      case Some(delta) =>
        ...
        alive.foreach { //
          (actorRef, _) =>
            actorRef ! DeltaMsg(ctx.self, tempCrdtState)
        }
        //Merge crdtState with tempCrdtState and keep the highest value for each key
        tempCrdtState.foreach {
          case (key, value) =>
            if (crdtstate.contains(key)) {
              if (crdtstate(key) < value) {
                crdtstate(key) = value
              }
            } else {
              crdtstate += (key -> value)
  ...
  private def writeCrdt(): Unit = {
    if (atomicMap.nonEmpty) {
      atomicMap.foreach { (key, value) =>
        tempCrdtState.put(key, value)
        //crdtstate = crdtstate.put(key, value)
      }
    ...
    while (opQueue.nonEmpty) {
      ...
      op match {
        case opPut(key, value) =>
         ...
         tempCrdtState.put(key, value)
        case opInc(key) =>
          //crdtstate = crdtstate.put(key, crdtstate.get(key).getOrElse(0) + 1)
          tempCrdtState.put(key, tempCrdtState.get(key).getOrElse(0) + 1)
  ...
  override def onMessage(msg: Command): Behavior[Command] = msg match
    ...
    case DeltaMsg(from, delta) =>
      //Merge delta with crdtstate and keep the highest value for each key
      delta.foreach {
        case (key, value) =>
          if (crdtstate.contains(key)) {
            if (crdtstate(key) < value) {
    ...
    case HeartBeat(from) =>
      if (!alive.contains(from)) {
        ctx.log.info(s"CRDTActor-$id received heartbeat from $from")
      }
    ...
    case Get(replyTo) =>
      ctx.log.info(s"CRDTActor-$id: My state is $crdtstate")
